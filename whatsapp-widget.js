(function() {
    ...
    widget.innerHTML = `
        <div id="wa-bubble">
            <div id="wa-text">¡Hola! ¿En qué puedo ayudarte?</div>
            <div id="wa-close" class="mi">close</div>
        </div>
        <a id="wa-button" href="https://wa.me/34666133977" target="_blank">
    ...
    // Intersection Observer to show widget when reaching contact/footer
    const target = document.getElementById('contact') || document.querySelector('.ag-footer');
    if (target) {
        const observer = new IntersectionObserver((entries) => {
            if (entries[0].isIntersecting) {
                ...
                setTimeout(() => {
                    bubble.classList.add('wa-active');
                    // Auto-hide bubble after 5 seconds
                    setTimeout(() => {
                        bubble.classList.remove('wa-active');
                    }, 5000);
                }, 800);
